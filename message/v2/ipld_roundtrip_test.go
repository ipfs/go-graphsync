...
func TestIPLDRoundTrip(t *testing.T) {
	...
	metadata := message.GraphSyncMetadatum{Link: root2, Action: graphsync.LinkActionMissing}

	responses := map[graphsync.RequestID]message.GraphSyncResponse{
		id1: message.NewResponse(id1, graphsync.RequestFailedContentNotFound, []message.GraphSyncMetadatum{metadata}),
	...
	require.True(t, ok)
	require.Len(t, gslm1.Clone(), 1)
	require.Equal(t, gslm1.Clone()[0], metadata)
	gslm2, ok := rtresmap[id2].Metadata().(message.GraphSyncLinkMetadata)
	require.True(t, ok)
	require.Empty(t, gslm2.Clone())
